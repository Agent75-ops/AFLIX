        ...
        <div className="carousel-container">
          <Slider className="carousel-moviespage" {...settings}>
            {iData.map((item)=>{
              ...
              return <SliderItem key={item.id} title={item.title} imgsrc={src} time={item.duration} genre={item.genre} dir={item.director} year={item.released} metaCritics={item.ratings.metacritics} imdb={item.ratings.imdb} plot={item.plot}/>
            })}
          </Slider>
        </div>
